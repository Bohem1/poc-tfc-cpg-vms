...
locals {
  name1  = substr(lower(replace(azurerm_resource_group.this.name, "/[^0-9a-zA-Z]/", "")), 0, 14)
  saname = format("sa%s%s", replace(local.name1, "/^rg/", ""), lower(random_string.project.result))
